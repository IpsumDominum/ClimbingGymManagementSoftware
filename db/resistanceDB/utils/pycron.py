    ...
    def next(self):
        ...
        while True:
            ...
            if self.last_run is not None and t.min == self.last_run:
                ...
                continue
            if (
                int(t.month) in self.schedule.months
                and int(t.day) in self.schedule.days
                and int(t.weekday) in self.schedule.weekdays
                and int(t.hour) in self.schedule.hours
                and int(t.min) in self.schedule.mins
                and int(t.sec) in [0, 1]
            ):
                self.last_run = t.min
                return True
            else:
        ...
        try:
            while self.next():
                ...
                try:
                    ...
                    raise RuntimeError(
                        "Could not start a process for task to run: {0:s}".format(e)
                    )
                p.join(self.timeout)
                if p.is_alive():
                    p.terminate()
    ...
    def __init__(self, time_tpl=None):
        ...
        if time_tpl is None:
            ...
            if isinstance(time_tpl[1], int):
                ...
                self.epoch_time = int(
                    time.mktime(time.strptime(time_tpl[1], self.format_string))
                )
...
if __name__ == "__main__":
    ...
    # cron schedule
    s = CronSchedule()
    s.hours = range(0, 24, 24)  # running every 24 hours
    # create the cron
    c = Cron(s)
